...
def _ask_groq_accuracy(
    ...
    try:
        ...
        return status if status in {"Accurate", "Inaccurate"} else "No Evidence Found"
...
def verify_claims(claims: List[str]) -> List[Dict]:
    ...
    For each claim:
      ...
      2. Passing evidence to Groq to decide Accurate / Inaccurate
    ...
    for claim in claims:
        ...
        # Step 3 - no source found 
        if not source_url:
            status = "No Evidence founc"
